...
3. **Then, and only then, add the CI check** that asserts each scaffolded app has the rules enabled. Landing that check before anyone has adopted would just make `main` red — the same sequencing mistake as documenting a script before writing it. The check belongs in the `docs` job added by [PR #34](https://github.com/rhaeyyan/riverside-books/pull/34).
