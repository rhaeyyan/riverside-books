...
- [x] **Assign ownership of event data.** No product owns an events table. Product C needs it ("Is the author event tonight?") and Product D's workbook specifies `Event: id, title, author/guest, date, time, description`. Product A's schema has none and explicitly declines tickets (`product-a/market_strategy.md:184`). This is the shared-Supabase question one layer down, and it will surface the same way — late. **Resolved:** Product A owns and migrates `events`, Product B writes to it, C and D read — see [`docs/schema.md`](docs/schema.md#events).
...
- [x] **Publish the shared schema field list.** Product D's Product Plan sheet lists this as a Team 13 decision and it blocks D's fact-protection design. **Resolved:** [`docs/schema.md`](docs/schema.md) is the single shared contract; A owns and migrates every table listed there.
...
- [x] **Decide the event-ticket question rather than carrying it.** It appeared as an open question in three places (`market_strategy.md:184`, `:196`, `implementation_plan.md:214`). **Resolved, split into two decisions:** ticket sales stay out of scope (unchanged), and event-*data* ownership goes to Product A regardless (see `docs/schema.md`). Whether a ticketed event should grant a loyalty stamp is intentionally left open — it's a separate loyalty-semantics question, not a data-ownership one.
...
- [x] **Explain or remove the CampaignFlow reference.** **Resolved:** both workbook references now describe the select → generate → review pattern directly, without an unexplained product name.
- [x] **Round the fit scores.** **Resolved:** the score formulas now round to integers, and the linked chart uses the rounded results.
- [x] **Export a text version of the workbook alongside the `.xlsx`.** **Resolved:** [`product-d/market_strategy.md`](product-d/market_strategy.md) is the diffable, source-linked Markdown version of the research and decisions.
...
- [x] **Convert the Product Plan sheet into `implementation_plan.md`.** **Resolved:** [`product-d/implementation_plan.md`](product-d/implementation_plan.md) turns the accepted MVP decisions into phases, tests, exit conditions, task decomposition, and explicit cross-team blockers.
