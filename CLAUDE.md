...
- **Open your own PR after pushing — don't have someone else open it for you.** GitHub attributes PR authorship to whoever calls `gh pr create` / clicks "New pull request", not whoever authored the commits. If rhaeyyan (or an agent acting as rhaeyyan) opens a PR on a teammate's behalf, GitHub treats rhaeyyan as the author and excludes them from their own required-reviewer list — the exact deadlock `CODEOWNERS` above exists to prevent. Push your branch, then run `gh pr create` (or use the GitHub UI) yourself, under your own account.
