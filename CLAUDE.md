...
This is a four-person team where each collaborator owns one product directory and runs their own build sessions independently — not one shared pairing session. The same lean roster applies to whichever product directory a session is scoped to, for teammates who choose to use it. Role-named agents (`.claude/agents/`, mirrored in `.codex/agents/*.toml`) so any teammate can tell what an agent does without cross-referencing a roster:
