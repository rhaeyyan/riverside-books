...
- **Open your own PR after pushing — don't have someone else open it for you.** GitHub attributes PR authorship to whoever calls `gh pr create` / clicks "New pull request", not whoever authored the commits. If Codex opens a PR under a different account than the one that pushed the branch, GitHub attributes it to whoever's session ran the command — which can exclude the intended teammate from their own required-reviewer list, the exact deadlock `CODEOWNERS` above exists to prevent. Push the branch, then run `gh pr create` (or use the GitHub UI) under the actual contributor's own account.
