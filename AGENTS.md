...
- **CI** (`.github/workflows/ci.yml`) runs lint, typecheck, test, and build on every push/PR, scoped per product as each app is scaffolded (see Commands above) — format-check and a coverage gate aren't wired into the workflow, only available as local commands. It is not yet a required status check on `main` — enable that once a given job first runs green:
