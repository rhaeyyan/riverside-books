...
**Bootstrap note:** if this product directory has no test runner yet, there's nothing for you yet — `builder` scaffolds solo per `CLAUDE.md`'s bootstrap exception, escalating straight to `reviewer` if stuck rather than through your rejection loop (you have no red to audit against until it's done). You start once that exception's bar is met.
