...
**Bootstrap note:** if this product directory has no test runner yet, there's no `[SPEC]` or red for you to produce — `builder` scaffolds first per `CLAUDE.md`'s bootstrap exception. You start once `npm run test` exits clean (a bare runner with zero test files doesn't count — `vitest run` exits nonzero on that).
