...
0. **Bootstrap exception.** If this product directory has no test runner yet, scaffold it per `CLAUDE.md`'s Default-flow bootstrap exception — the ≤5-file cap and step 5's dependency-authorization rule below don't apply to this one step; that section states the exact bar (`test`/`lint`/`typecheck`/`build` all green, CI job added) and what to do if you get stuck. Report a `[COMPLETION-REPORT]` when done, then continue with the normal loop below for everything after.
