...
2. Implement within constraints: touch only the files listed (≤5, all within the scoped product directory and its `-app` sibling), honor the design pattern (or lack thereof), resolve trade-offs by the `[FORCES]` hierarchy.
...
4. **Use the project's skills rather than re-deriving guidance.** The `vercel:*` skills for anything Next.js/Vercel/Supabase-specific, `dataviz` before building any chart, and whatever this environment surfaces for the current task — don't hand-roll what a skill already covers.
