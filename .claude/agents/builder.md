...
0. **Bootstrap exception — no test runner yet.** If this product directory has no `package.json`/test runner (Products A, B, D before their own Phase 0 lands), there's nothing for `sdet` to produce a red against — scaffold directly per `CLAUDE.md`'s bootstrap exception: project init, `package.json`, ESLint/Vitest/Tailwind config, until `npm run test` runs (even on zero tests). Report what you built in a `[COMPLETION-REPORT]` noting no `[SPEC]` applied. The moment the test runner exists, stop using this exception — every task after it, including the first real feature, goes through the normal loop below.
